...
(function (root) {
  ...
  function patch(old, next) {
    ...
    if (old.matches('input,textarea')) {
      if (!focused && old.value !== next.value) old.value = next.value;
      if ('checked' in old) old.checked = next.checked;
      old.disabled = next.disabled;
      return;
    }
    const pool = new Map(Array.from(old.childNodes).filter(n => key(n)).map(n => [key(n), n]));
    let cursor = old.firstChild;
    for (const child of Array.from(next.childNodes)) {
      let match = key(child) ? pool.get(key(child)) : cursor;
      if (!compatible(match, child)) match = null;
      if (!match) {
        match = child.cloneNode(true);
